...
| 互動 | 規則 |
|-----|------|
| 可點擊節點 | 點擊中間節點導向對應層級頁面；最後節點（當前頁）不可點擊（`cursor: default`，無 `<a>` / `<button>` 包裹） |
| 頁面標題聯動 | Breadcrumb 最後節點隨當前頁標題即時更新；單號為空（新單）時顯示 `—` |
| 路徑層數 | ERP 固定三層（模組分類 > 功能名稱 > 單號），不超過 5 層；超過 5 層改折疊中間節點（顯示 `…`） |
| 折疊展開 | 折疊後點擊 `…` 展開完整路徑，不觸發頁面導航 |
| 歷史狀態保留 | 從 Breadcrumb 返回上一層時，須恢復先前的篩選條件、分頁位置與捲動狀態 |
