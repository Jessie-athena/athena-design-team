...
- 結構從上到下：
  1. `.nav-rail__top` — **產品縮寫文字**（如「ERP」），下方 `1px solid var(--border-strong)` 分隔線
  2. `.nav-rail__items` — 主要 nav 按鈕清單（`flex: 1`）
  3.（可選）底部 footer action 區
...
> **必含產品縮寫**文字（如「ERP」），位置在 `.nav-rail__top`，**不可遺失**。
