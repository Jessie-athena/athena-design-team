...
> **Why**：「這個模組不是傳統 ERP 作業/設定檔」是關於整頁定位的判斷，理論上可能被過度推廣成「這個模組不適用 canonical 資產」。這條規則是防禦性規範——2026-07-06 的控制測試中，單獨抽出「非 ERP」這個變數並未讓元件層被連坐放棄，目前沒有證據顯示這是一個會獨立發生的問題（詳見 `pitfalls.md` 對應條目）；保留本規則是為了防患未然、成本低，不代表已確認的必然故障模式。
